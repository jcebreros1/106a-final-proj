...
def rotation_3d(omega, theta):
    ...
    ident = np.eye(3)
    normOfVector = math.sqrt(omega[0]**2 + omega[1]**2 + omega[2]**2)
    firstScale = np.sin(normOfVector * theta) / normOfVector
    secondScale = (1 - np.cos(normOfVector * theta)) / (normOfVector**2)
    output = ident + (firstScale * skew_3d(omega)) + (secondScale * np.matmul(skew_3d(omega), skew_3d(omega)))
...
def homog_3d(xi, theta):
    ...
    if np.allclose(w, 0):
        ...
        normOfVector = math.sqrt(w[0]**2 + w[1]**2 + w[2]**2)
        firstVal = np.matmul((np.eye(3) - rotation_3d(w, theta)), (np.matmul(skew_3d(w), v)))
        secondVal = np.matmul(np.matmul(w.reshape(3,1), w.reshape(1,3)), v) * theta
        p = (1/(normOfVector**2)) * (firstVal + secondVal)
    g = np.eye(4)
    g[:3, :3] = R
    g[:3, 3] = p
    return g
...
def prod_exp(xi, theta):
    ...
    #pass
    xiVals = np.array([xi[0][0], xi[1][0], xi[2][0], xi[3][0], xi[4][0], xi[5][0]])
    output = homog_3d(xiVals, theta[0])
    for x in range(1, len(xi[0])):
        xiVals = np.array([xi[0][x], xi[1][x], xi[2][x], xi[3][x], xi[4][x], xi[5][x]])
        output = np.matmul(output, homog_3d(xiVals, theta[x]))
